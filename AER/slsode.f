c-----------------------------------------------------------------------
c this is the single precision version of lsode from www.netlib.org
c modifications:
c  - no linpac subroutines included
c  - changed the following subroutine names by appending a "s"
c    cfode ewset intdy prepj solsy stode vnorm xsetf xsetun srcom
c    lsode changed to slsode, xerrwv changed to xerrws,
c    to make them unique to the single precission version
c    likewise changed common block /ls0001/ to /lss001/
c    likewise changed common block /eh0001/ to /ehs001/
c    changes are only made in code lines, not in comment lines
cgy
c-----------------------------------------------------------------------
...
c         /ehs001/  of length  2 (integer words).
c
...
c in subroutines lsode and xerrwv, which load variables into these
...
c note..  vnorm, isamax, sdot, and r1mach are function routines.
...
c load and invert the ewt array.  (h is temporarily set to 1.0.) -------
      nq = 1
      h = 1.0e0
      call ewsets (n, itol, rtol, atol, rwork(lyh), rwork(lewt))
      do 120 i = 1,n
...
c-----------------------------------------------------------------------
 ...
 160  tol = amax1(tol,100.0e0*uround)
      ...
      sum = vnorms (n, rwork(lf0), rwork(lewt))
...
c-----------------------------------------------------------------------
 ...
 210  if ((tn - tout)*h .lt. 0.0e0) go to 250
      call intdys (tout, 0, rwork(lyh), nyh, y, iflag)
...
c-----------------------------------------------------------------------
 ...
 270  tolsf = uround*vnorms (n, rwork(lyh), rwork(lewt))
 ...
 280  if ((tn + h) .ne. tn) go to 290
      ...
      call xerrws(50hlsode--  warning..internal t (=r1) and h (=r2) are,
     1   50, 101, 0, 0, 0, 0, 0, 0.0e0, 0.0e0)
      call xerrws(
     1  60h      such that in the machine, t + h = t on the next step  ,
     1   60, 101, 0, 0, 0, 0, 0, 0.0e0, 0.0e0)
      call xerrws(50h      (h = step size). solver will continue anyway,
     1   50, 101, 0, 0, 0, 0, 2, tn, h)
      if (nhnil .lt. mxhnil) go to 290
      call xerrws(50hlsode--  above warning has been issued i1 times.  ,
     1   50, 102, 0, 0, 0, 0, 0, 0.0e0, 0.0e0)
      call xerrws(50h      it will not be issued again for this problem,
     1   50, 102, 0, 1, mxhnil, 0, 0, 0.0e0, 0.0e0)
 290  continue
c-----------------------------------------------------------------------
c     call stode(neq,y,yh,nyh,yh,ewt,savf,acor,wm,iwm,f,jac,prepj,solsy)
c-----------------------------------------------------------------------
      call stodes (neq, y, rwork(lyh), nyh, rwork(lyh), rwork(lewt),
     1   rwork(lsavf), rwork(lacor), rwork(lwm), iwork(liwm),
     2   f, jac, prepjs, solsys)
      kgo = 1 - kflag
...
c the maximum number of steps was taken before reaching tout. ----------
 500  call xerrws(50hlsode--  at current t (=r1), mxstep (=i1) steps   ,
     1   50, 201, 0, 0, 0, 0, 0, 0.0e0, 0.0e0)
      call xerrws(50h      taken on this call before reaching tout     ,
     1   50, 201, 0, 1, mxstep, 0, 1, tn, 0.0e0)
      istate = -1
      go to 580
c ewt(i) .le. 0.0 for some i (not at start of problem). ----------------
 510  ewti = rwork(lewt+i-1)
      call xerrws(50hlsode--  at t (=r1), ewt(i1) has become r2 .le. 0.,
     1   50, 202, 0, 1, i, 0, 2, tn, ewti)
      istate = -6
      go to 580
c too much accuracy requested for machine precision. -------------------
 520  call xerrws(50hlsode--  at t (=r1), too much accuracy requested  ,
     1   50, 203, 0, 0, 0, 0, 0, 0.0e0, 0.0e0)
      call xerrws(50h      for precision of machine..  see tolsf (=r2) ,
     1   50, 203, 0, 0, 0, 0, 2, tn, tolsf)
      rwork(14) = tolsf
      istate = -2
      go to 580
c kflag = -1.  error test failed repeatedly or with abs(h) = hmin. -----
 530  call xerrws(50hlsode--  at t(=r1) and step size h(=r2), the error,
     1   50, 204, 0, 0, 0, 0, 0, 0.0e0, 0.0e0)
      call xerrws(50h      test failed repeatedly or with abs(h) = hmin,
     1   50, 204, 0, 0, 0, 0, 2, tn, h)
      istate = -4
      go to 560
c kflag = -2.  convergence failed repeatedly or with abs(h) = hmin. ----
 540  call xerrws(50hlsode--  at t (=r1) and step size h (=r2), the    ,
     1   50, 205, 0, 0, 0, 0, 0, 0.0e0, 0.0e0)
      call xerrws(50h      corrector convergence failed repeatedly     ,
     1   50, 205, 0, 0, 0, 0, 0, 0.0e0, 0.0e0)
      call xerrws(30h      or with abs(h) = hmin   ,
...
c the run is halted.
c-----------------------------------------------------------------------
 601  call xerrws(30hlsode--  istate (=i1) illegal ,
     1   30, 1, 0, 1, istate, 0, 0, 0.0e0, 0.0e0)
      go to 700
 602  call xerrws(30hlsode--  itask (=i1) illegal  ,
     1   30, 2, 0, 1, itask, 0, 0, 0.0e0, 0.0e0)
      go to 700
 603  call xerrws(50hlsode--  istate .gt. 1 but lsode not initialized  ,
     1   50, 3, 0, 0, 0, 0, 0, 0.0e0, 0.0e0)
      go to 700
 604  call xerrws(30hlsode--  neq (=i1) .lt. 1     ,
     1   30, 4, 0, 1, neq(1), 0, 0, 0.0e0, 0.0e0)
      go to 700
 605  call xerrws(50hlsode--  istate = 3 and neq increased (i1 to i2)  ,
     1   50, 5, 0, 2, n, neq(1), 0, 0.0e0, 0.0e0)
      go to 700
 606  call xerrws(30hlsode--  itol (=i1) illegal   ,
     1   30, 6, 0, 1, itol, 0, 0, 0.0e0, 0.0e0)
      go to 700
 607  call xerrws(30hlsode--  iopt (=i1) illegal   ,
     1   30, 7, 0, 1, iopt, 0, 0, 0.0e0, 0.0e0)
      go to 700
 608  call xerrws(30hlsode--  mf (=i1) illegal     ,
     1   30, 8, 0, 1, mf, 0, 0, 0.0e0, 0.0e0)
      go to 700
 609  call xerrws(50hlsode--  ml (=i1) illegal.. .lt.0 or .ge.neq (=i2),
     1   50, 9, 0, 2, ml, neq(1), 0, 0.0e0, 0.0e0)
      go to 700
 610  call xerrws(50hlsode--  mu (=i1) illegal.. .lt.0 or .ge.neq (=i2),
     1   50, 10, 0, 2, mu, neq(1), 0, 0.0e0, 0.0e0)
      go to 700
 611  call xerrws(30hlsode--  maxord (=i1) .lt. 0  ,
     1   30, 11, 0, 1, maxord, 0, 0, 0.0e0, 0.0e0)
      go to 700
 612  call xerrws(30hlsode--  mxstep (=i1) .lt. 0  ,
     1   30, 12, 0, 1, mxstep, 0, 0, 0.0e0, 0.0e0)
      go to 700
 613  call xerrws(30hlsode--  mxhnil (=i1) .lt. 0  ,
     1   30, 13, 0, 1, mxhnil, 0, 0, 0.0e0, 0.0e0)
      go to 700
 614  call xerrws(40hlsode--  tout (=r1) behind t (=r2)      ,
     1   40, 14, 0, 0, 0, 0, 2, tout, t)
      call xerrws(50h      integration direction is given by h0 (=r1)  ,
     1   50, 14, 0, 0, 0, 0, 1, h0, 0.0e0)
      go to 700
 615  call xerrws(30hlsode--  hmax (=r1) .lt. 0.0  ,
     1   30, 15, 0, 0, 0, 0, 1, hmax, 0.0e0)
      go to 700
 616  call xerrws(30hlsode--  hmin (=r1) .lt. 0.0  ,
 ...
 618  call xerrws(
     ...
     1   60, 18, 0, 2, leniw, liw, 0, 0.0e0, 0.0e0)
      go to 700
 619  call xerrws(40hlsode--  rtol(i1) is r1 .lt. 0.0        ,
     1   40, 19, 0, 1, i, 0, 1, rtoli, 0.0e0)
      go to 700
 620  call xerrws(40hlsode--  atol(i1) is r1 .lt. 0.0        ,
     1   40, 20, 0, 1, i, 0, 1, atoli, 0.0e0)
      go to 700
 621  ewti = rwork(lewt+i-1)
      call xerrws(40hlsode--  ewt(i1) is r1 .le. 0.0         ,
 ...
 625  call xerrws(
     ...
     1   60, 25, 0, 0, 0, 0, 2, tcrit, tout)
      go to 700
 626  call xerrws(50hlsode--  at start of problem, too much accuracy   ,
     ...
     1   60, 26, 0, 0, 0, 0, 1, tolsf, 0.0e0)
      rwork(14) = tolsf
      go to 700
 627  call xerrws(50hlsode--  trouble from intdy. itask = i1, tout = r1,
...
c
 700  if (illin .eq. 5) go to 710
      ...
      istate = -3
      return
 710  call xerrws(50hlsode--  repeated occurrences of illegal input    ,
     1   50, 302, 0, 0, 0, 0, 0, 0.0e0, 0.0e0)
c
 800  call xerrws(50hlsode--  run aborted.. apparent infinite loop     ,
     1   50, 303, 2, 0, 0, 0, 0, 0.0e0, 0.0e0)
      return
c----------------------- end of subroutine lsode -----------------------
      end

      subroutine cfodes (meth, elco, tesco)
clll. optimize
      integer meth
...
c----------------------- end of subroutine cfode -----------------------
      ...
      subroutine ewsets (n, itol, rtol, atol, ycur, ewt)
clll. optimize
c-----------------------------------------------------------------------
...
c----------------------- end of subroutine ewset -----------------------
      ...
      subroutine intdys (t, k, yh, nyh, dky, iflag)
clll. optimize
      integer k, nyh, iflag
      integer iownd, iowns,
...
c
 ...
 60     dky(i) = r*dky(i)
      return
c
 80   call xerrws(30hintdy--  k (=i1) illegal      ,
     1   30, 51, 0, 1, k, 0, 0, 0.0e0, 0.0e0)
      iflag = -1
      return
 90   call xerrws(30hintdy--  t (=r1) illegal      ,
...
c----------------------- end of subroutine intdy -----------------------
      ...
      subroutine prepjs (neq, y, yh, nyh, ewt, ftem, savf, wm, iwm,
     1   f, jac)
clll. optimize
      external f, jac
      integer neq, nyh, iwm
...
c----------------------- end of subroutine prepj -----------------------
      ...
      subroutine solsys (wm, iwm, x, tem)
clll. optimize
      integer iwm
      integer iownd, iowns,
     1   icf, ierpj, iersl, jcur, jstart, kflag, l, meth, miter,
...
c----------------------- end of subroutine solsy -----------------------
      ...
      subroutine stodes (neq, y, yh, nyh, yh1, ewt, savf, acor,
...
clll. optimize
     ...
     1   r, rh, rhdn, rhsm, rhup, told, vnorms
      dimension neq(1), y(1), yh(nyh,1), yh1(1), ewt(1), savf(1),
     1   acor(1), wm(1), iwm(1)
      common /lss001/ conit, crate, el(13), elco(13,12),
...
c-----------------------------------------------------------------------
 ...
 125    el(i) = elco(i,nq)
      ...
      ddn = vnorms (n, savf, ewt)/tesco(1,l)
      exdn = 1.0e0/float(l)
...
c-----------------------------------------------------------------------
 140  call cfodes (meth, elco, tesco)
 150  do 155 i = 1,l
 155    el(i) = elco(i,nq)
      nqnyh = nq*nyh
      rc = rc*el(1)/el0
...
c-----------------------------------------------------------------------
 450  jcur = 0
      ...
      if (m .gt. 0) dsm = vnorms (n, acor, ewt)/tesco(2,nq)
...
c-----------------------------------------------------------------------
 ...
 530    savf(i) = acor(i) - yh(i,lmax)
      dup = vnorms (n, savf, ewt)/tesco(3,nq)
 ...
 540  exsm = 1.0e0/float(l)
      ...
      ddn = vnorms (n, yh(1,l), ewt)/tesco(1,nq)
      exdn = 1.0e0/float(nq)
      rhdn = 1.0e0/(1.3e0*ddn**exdn + 0.0000013e0)
...
c-----------------------------------------------------------------------
 ...
 10     sum = sum + (v(i)*w(i))**2
      vnorms = sqrt(sum/float(n))
      return
c----------------------- end of function vnorm -------------------------
      end

      subroutine xerrws (msg, nmes, nerr, level, ni, i1, i2, nr, r1, r2)
...
c    in e21.13 format.
c 5. the common block /ehs001/ below is data-loaded (a machine-
...
c abort the run if level = 2. ------------------------------------------
 100  if (level .ne. 2) return
      stop 'A level is equal to 2 in slsode.f'
c----------------------- end of subroutine xerrwv ----------------------
      end

      subroutine srcoms (rsav, isav, job)
c-----------------------------------------------------------------------
c this routine saves or restores (depending on job) the contents of
c the common blocks ls0001 and ehs001, which are used internally
