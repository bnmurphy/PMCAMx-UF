...
c
      ...
      character*198 filena
      character*198  filena_lat,filena_lon      
...
c        endif
      ...
      enddo
       ...
       filena_lat='LatLon/lat.dat'
       open(27,file=filena_lat,action='write')
       write(27,'(150f8.2)'),((cellat(i,j),i=1,ncol),j=1,nrow)

       filena_lon='LatLon/lon.dat'
       open(37,file=filana_lon,action='write')
       write(37,'(150f8.2)'),((cellon(i,j),i=1,ncol),j=1,nrow)
...
cbin      if (time.eq.5.and.date.eq.8122) then 
cbin      filena='cldtrnsDir/cldtrns.'//julday(idate)//
cbin     &       '.ly'//clay(k)//'.bin'
cbin      open(unit=31,file=filena, action='write',status='new',
cbin     & form='unformatted')

cbin        write(31),((cldtrns(i,j,k),i=1,ncol),j=1,nrow)
c end the add by Elham
cbin          endif
