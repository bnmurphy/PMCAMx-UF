...
c
      ...
      tcpu = dtime(tarray2)
c
      call checkconc(conc,ncol,nrow,nlay,nspc,species,2)
cdbg      write(*,*)'Concentration of NUM_18 at 97,2,1',conc(97,2,1,611)
c
      do 20 k = 1,nlay
        dtuse = deltat/nadv(k)
...
cdbg            endif
c
            call mnratio(ii,ngas,naero,c1d,c2d,i1,i2,MX1D,mn) ! by jgj 8/13/06
c
...
c
      call checkconc(conc,ncol,nrow,nlay,nspc,species,3)
c
      do 40 k = 1,nlay
        dtuse = deltat/nadv(k)
