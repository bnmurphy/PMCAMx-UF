c**** STARTDDM.F
c
      subroutine startddm(iounit,iout,nopen)
c
c-----CAMx v4.02 030709
c
c-----------------------------------------------------------------------
...
c
      do 10 igrd=1,ngrid
         write(action,'(2A,I2)') 'Reading filename for source area ',
     &                                     'map for DDM.  Grid: ',igrd
...
c
      action = 'Reading filename for receptor definition for DDM '
...
c
      if( lrstrt ) then
         action = 'Reading the DDM coarse grid instantaneous file.'
     ...
     &                          inifil(IDXCRS)(:istrln(inifil(IDXCRS)))
         if( ngrid .GT. 1 ) then
            action = 'Reading the DDM fine grid instantaneous file.'
...
c
      if( nicddm .GT. 0 .AND. .NOT. lrstrt ) then
         action = 'Reading the DDM IC file.'
         read(iounit,'(20X,A)',END=7003) icfil
         fname = icfil
         inquire(file=icfil,exist=lexist)
         if( .NOT. lexist ) goto 7000
         nopen =  nopen + 1
         write(iout,9000)'Initial conditions file for DDM      (unit):',
     &                                                            IORIC
         write(iout,9002) '   File: ',icfil(:istrln(icfil))
      else
         action = 'Reading the dummy DDM IC file.'
         read(iounit,'(A)',END=7003) 
      endif
c
c   --- BC file and TOPCONC file for DDM ---
c
      if( nbcddm .GT. 0 ) then
         action = 'Reading the DDM lateral BC file.'
         read(iounit,'(20X,A)',END=7003) bcfil
         fname = bcfil
         inquire(file=bcfil,exist=lexist)
         if( .NOT. lexist ) goto 7000
         nopen =  nopen + 1
         write(iout,9000)'Boundary conditions file for DDM     (unit):',
     &                                                            IORBC
         write(iout,9002) '   File: ',bcfil(:istrln(bcfil))
c
         action = 'Reading the DDM top concentration file.'
         read(iounit,'(20X,A)',END=7003) tcfil
         fname = tcfil
         inquire(file=tcfil,exist=lexist)
         if( .NOT. lexist ) goto 7000
         nopen =  nopen + 1
         write(iout,9000)'Top conc conditions file for DDM     (unit):',
     &                                                            IORTC
         write(iout,9002) '   File: ',tcfil(:istrln(tcfil))
      else
         action = 'Reading the dummy DDM lateral BC file.'
         read(iounit,'(A)',END=7003) 
         action = 'Reading the dummy DDM top concentration file.'
         read(iounit,'(A)',END=7003) 
      endif
c
c   --- emissions files for the source groupings,
c       slip if not doing any emissions DDM species ---
c
...
c
         do 30 j=1,ngrid
             action = 'Reading a DDM surface emissions file.'
...
c
         action = 'Reading the DDM point source emissions file.'
...
c
         ...
         fname = cn1fil(i)
         nopen = nopen + 1
         action = 'Opening first DDM instantaneous file.'
...
c
         ...
         fname = cn2fil(i)
         nopen = nopen + 1
         action = 'Opening second DDM instantaneous file.'
...
c
      ...
      fname = avgfil
      nopen = nopen + 1
      action = 'Opening DDM receptor output file.'
...
c
 7003 continue
     ...
     &                  'CAMx.in file when reading DDM filenames.'
      call camxerr()
c
 7004 continue
      write(iout,'(//,A)') 'ERROR in STARTDDM:'
