...
c        begtim              beginning time of simulation  by jgj 02/23/2007
c
c     Output arguments:
c        irec                line number in control file
c
...
c
      nopen = 0
      if (begtim .eq. 0) then ! jgj 3/23/2007
      filroot(ii+1:) = '.inst.1'
      iconc(1) = 10 
      nopen = nopen + 1
...
cjgj      open(unit=ichem,file=filtmp,status='OLD')
...
cjgj      write(iout,'(2A)') 'Could not open file: ',
cjgj     &                                   filtmp(:istrln(filtmp))
      write(iout,'(3A,I2)') 'Could not open file: ',
