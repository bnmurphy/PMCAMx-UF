...
#	make model FC="pgf77" FLGS="-I$(INC) -g -tp k8-64 -pc 64 -Mnoframe -byteswapio -Wl,-Bstatic" TARGT="CAMx.$(DOMAIN).linux" DUM=dummy
	...
	make model FC="pgf77" FLGS="-I$(INC) -g -tp k8-64 -pc 64 -Mnoframe -Mextend -byteswapio -Wl, -mcmodel=medium" TARGT="CAMx.$(DOMAIN).linux" DUM=dummy

#	make model FC="pgf77" FLGS="-I$(INC) -g -tp k8-64 -pc 64 -Mnoframe -Mextend -byteswapio -Wl" TARGT="CAMx.$(DOMAIN).linux" DUM=dummy
...
clean:	
	rm -f $(OBJCTS) dummy*.o CAMx.ladco.linux
...
numconv.o		: numconv.f                                            \
                        $(INC)/camx.prm $(INC)/camx.com                        \
                        $(INC)/section.inc $(INC)/grid.com $(INC)/chmstry.com  \
                        $(INC)/aerpar.inc $(INC)/diameters.inc
...
$(BNRY)/readcnc.o 	: $(BNRY)/readcnc.f                                    \
                        $(INC)/camx.prm $(INC)/camx.com $(INC)/camxfld.com     \
                        $(INC)/filunit.com $(INC)/bndary.com $(INC)/grid.com   \
                        $(INC)/chmstry.com $(INC)/flags.com $(INC)/section.inc \
                        $(INC)/diameters.inc
...
$(DMAN)/alpha_nh3.o     : $(DMAN)/alpha_nh3.f

$(DMAN)/dmdt_int.o      : $(DMAN)/dmdt_int.f

$(DMAN)/dman.o          : $(DMAN)/dman.f                                       \
                        $(INC)/sizecode.COM $(INC)/aervaria.inc

$(DMAN)/gasdiff.o  	: $(DMAN)/gasdiff.f

$(DMAN)/initbounds.o	: $(DMAN)/initbounds.f                                 \
			$(INC)/sizecode.COM

$(DMAN)/multicoag.o     : $(DMAN)/multicoag.f                                  \
	                $(INC)/sizecode.COM $(INC)/aervaria.inc
