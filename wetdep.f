     ...
     &                       cgas,dscav,gscav,gdiff,ascav)
              ...
              elseif (kph2o_1.ne.nspec+1) then ! mechanism 6
                kwtr = (kph2o_1 - ngas) / nsec + 1
                   ! kwtr is the order of water in aerosol species.
                knum = (knum_1 - ngas) / nsec + 1
                   ! knum is the order of number conc. in aerosol species.
                   ! by jgj 2/20/06
                ...
                do isec = 1, nsec
                  ...
                  qt = 0.0
                  roprta = 0.0
cjgj                  do iaero = 1, naero
                  do iaero = 1, naero-1 ! skip number conc. by jgj 10/13/06
cjgj                    if (iaero.ne.kwtr) then
                    if ((iaero.ne.kwtr).and.(iaero.ne.knum)) then
...
c<-
              do 50 l = ngas+1,nspec
                ...
                if (ltop) tmass(l) = 0.
cjgj                cmin = bdnl(l)
cjgj                conc(i,j,k,l) = amax1(cmin,conc(i,j,k,l))
                   ! TOMAS algorithm requires M/N ratio within size bins. This cmin treatment causes a deviation of M/N ratio. It is because concentrations are pretty small due to a high resolution. It has been commented out by jgj 10/19/06.
...
c<-
cjgj                delc = amin1(delc,conc(i,j,k,l)-cmin) ! TOMAS algorithm requires M/N ratio within size bins. This cmin treatment causes a deviation of M/N ratio. It is because concentrations are pretty small due to a high size resolution. It has been commented out by jgj 10/19/06.
c
                conc(i,j,k,l) = conc(i,j,k,l) - delc
                delm = delc*cellvol
                tmass(l) = tmass(l) + delm
