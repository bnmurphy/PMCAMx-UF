...
C       4. distn.out - a "MAIN OUTPUT" file, prints number concentration 
C                     as dN/dlogDp
...
C-----Print every interval set by kprint
      ...
      ! 4. distn.dat
      kkprint=INT(kprint/(INT(1.001*deltat*3600.)/15))
      if (kcount .ge. kkprint) then
...
C-----Print for analytic solutions
      ...
      ! For coagulation tests, print every at 0:00, 0:24 and 0:84.
        if (icoag_test .eq. 1) then
          if ((time.eq.tstart).or.((time.le.24./60.+deltat/2.).and.
     &      (time.gt.24./60.-deltat/2.)).or.((time.le.84./60.+deltat/2.)
     &      .and.(time.gt.84./60.-deltat/2.))) then
