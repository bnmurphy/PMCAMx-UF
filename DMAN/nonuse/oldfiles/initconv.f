...
C-----Mass of particles from input files
      do i=1,nsect+1
        xko(i)=1.*pi6*dpbound(i)**3.*density*1.0e-18 ! kg
      enddo
...
C-----Initialize mass concentrations
      ...
      !Assume half of mass consist of ammonium sulfate 
      do k=1,ibins
        !0.72723 is a fraction of sulfate in ammonium sulfate
        Mk(k,srtso4)=0.5*1.4*xk(k)*Nk(k)*0.727273  ! sulfate 
        Mk(k,srtorg)=0.5*1.4*xk(k)*Nk(k)  ! organic matter
        !0.272727 is a fraction of ammonium in ammonium sulfate
        Mk(k,srtnh3)=0.5*1.4*xk(k)*Nk(k)*0.272727 ! ammonium
